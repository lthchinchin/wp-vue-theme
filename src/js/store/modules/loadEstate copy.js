export const loadEstate = {
    ...
    actions: {
        ...
        sortByFilter({ commit, state }, para) {
            state.dataPara.page = 1
            console.log('test para api ', para);
            commit('setIsLoading', true);
            let stateTmp = state
            if (Object.keys(para)[0] == 'price') {
                ...
                if (Object.keys(para)[0] == 'area') {
                    ...
                    let keyName = Object.keys(para)[0]
                    if (keyName == 'post_type') {
                        commit('setEstateType', para[keyName]);
                    }
                    let attrSave = parseInt(para[keyName])
                    console.log('attrSave', attrSave);
                    if (!attrSave && para[keyName] == '') {
                        //khi gtri cua option tat ca la '' , xoa attr object search para  
                        const { [keyName]: propValue, ...rest } = state.dataPara;
                        state.dataPara = rest;
                    } else {
                        if (attrSave) {
                            state.dataPara[keyName] = attrSave
                        } else {
                            state.dataPara[keyName] = para[keyName]
                        }
                    }
                }
            }
